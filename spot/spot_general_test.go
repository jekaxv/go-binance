...
type spotGeneralTestSuite struct {
	baseHttpTestSuite
}

func TestApiGeneralAccount(t *testing.T) {
	suite.Run(t, new(spotGeneralTestSuite))
}

func (s *spotGeneralTestSuite) TestNewPing() {
	msg := []byte(`{}`)
	server := s.setup(msg)
	defer server.Close()
...
func (s *spotGeneralTestSuite) TestNewServerTime() {
	msg := []byte(`{
...
func (s *spotGeneralTestSuite) TestNewExchangeInfo() {
	msg := []byte(`{
  "timezone": "UTC",
...
func (s *spotGeneralTestSuite) assertTestExchangeInfoResponse(r1, r2 *ExchangeInfoResponse) {
	r := s.r()
