...
func (s *apiMarketTestSuite) TestNewKlineData() {
	...
	server := s.setup(msg)
	defer server.Close()
	resp, err := s.client.NewKline().Symbol("BTCUSDT").Interval(types.Interval1m).Do(context.Background())
	r := s.r()
	r.Empty(err)
...
func (s *apiMarketTestSuite) TestNewContractKline() {
	...
	server := s.setup(msg)
	defer server.Close()
	resp, err := s.client.NewContractKline().Pair("BTCUSDT").ContractType(types.ContractTypePERPETUAL).Interval(types.Interval1m).Do(context.Background())
	r := s.r()
	r.Empty(err)
	testResp := []*KlineDataResponse{{
		OpenTime:                 1737556140000,
...
func (s *apiMarketTestSuite) TestNewIndexKline() {
	...
	server := s.setup(msg)
	defer server.Close()
	resp, err := s.client.NewIndexKline().Pair("BTCUSDT").Interval(types.Interval1m).Do(context.Background())
	r := s.r()
	r.Empty(err)
	testResp := []*KlineDataResponse{{
		OpenTime:                 1737556140000,
...
func (s *apiMarketTestSuite) TestNewMarkKline() {
	...
	server := s.setup(msg)
	defer server.Close()
	resp, err := s.client.NewMarkKline().Symbol("BTCUSDT").Interval(types.Interval1m).Do(context.Background())
	r := s.r()
	r.Empty(err)
	testResp := []*KlineDataResponse{{
		OpenTime:                 1737556140000,
...
func (s *apiMarketTestSuite) TestNewPremiumKline() {
	...
	server := s.setup(msg)
	defer server.Close()
	resp, err := s.client.NewPremiumKline().Symbol("BTCUSDT").Interval(types.Interval1m).Do(context.Background())
	r := s.r()
	r.Empty(err)
	testResp := []*KlineDataResponse{{
		OpenTime:                 1737556140000,
