...
func (s *accountTestSuite) TestNewAccountBalance() {
	msg := []byte(`{
	...
	r := s.r()
	r.Empty(err)
	var testResp *AccountBalanceResponse
	r.Empty(json.Unmarshal(msg, &testResp))
	s.assertTestAccountBalanceResponse(resp, testResp)
}

func (s *accountTestSuite) assertTestAccountBalanceResponse(r1, r2 *AccountBalanceResponse) {
...
func (s *accountTestSuite) assertMarketAccountBalanceResult(r1, r2 *AccountBalanceResult) {
	r := s.r()
...
func (s *accountTestSuite) assertAccountInfoResult(r1, r2 *AccountInfoResult) {
	r := s.r()
